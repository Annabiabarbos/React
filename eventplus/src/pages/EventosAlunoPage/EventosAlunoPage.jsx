...
const EventosAlunoPage = () => {
  ...
  const commentaryRemove = async (userId) => {
  ...
  async function newCommentary(situacao, comentaryText) {
    try {
      const postEvent = await api.post(CommentaryEventResource, {
        descricao: comentaryText,
        exibe: situacao,
        idUsuario: userData.userId,
        // "idEvento": eventId
      });
      notifySuccess("Comentário Realizado");
  ...
  async function handleConnect(eventId, functionConnect, presenceId = null) {
    ...
    try {
      ...
      const unconnected = await api.delete(
        `${presenceEventResource}/${presenceId}`
      );
      ...
      notifySuccess("Seu cadastro foi desfeito");
